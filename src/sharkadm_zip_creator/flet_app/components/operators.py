...
class OperatorCard(ft.Card):
    ...
    def init(self):
        ...
        self.content = ft.Container(
            content=ft.Row([self._main_cb, self._children_col]),
            width=400,
            padding=10,
            expand=True,
        )
    ...
    def get_info(self) -> dict:
        ...
        for key, wid in self.operator_widgets.items():
            value = wid.value
            if (
                hasattr(wid, "input_filter")
                and wid.input_filter
                and isinstance(wid.input_filter, ft.NumbersOnlyInputFilter)
            ):
                value = int(value)
            info[key] = value
        return info
